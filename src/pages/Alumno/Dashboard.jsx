...
import { FaDumbbell, FaUtensils, FaEnvelope, FaPlayCircle, FaCheckCircle, FaArrowRight, FaChevronDown } from 'react-icons/fa';
...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        const fetchPerfilYRutinas = async () => {
            ...
            const { data: sesionesCompletadas } = await supabase
            ...
            sesionesCompletadas?.forEach(s => {
                if (s.rutina_personalizada_id) idsCompletados.add(`p-${s.rutina_personalizada_id}`);
                if (s.rutina_base_id) idsCompletados.add(`b-${s.rutina_base_id}`);
            });
            ...
            const startOfWeek = new Date(); startOfWeek.setDate(hoy.getDate() - ((hoy.getDay() + 6) % 7)); startOfWeek.setHours(0, 0, 0, 0);
    ...
    const iniciarRutina = useCallback(rutina => { setSelectedRutina(rutina); setIsSheetOpen(true); }, []);
    const handleCloseSheet = useCallback(() => { setIsSheetOpen(false); setTimeout(() => setSelectedRutina(null), 300); }, []);
    const rutinaHoy = useMemo(() => rutinas.find(r => r.dia === todayIndex), [rutinas, todayIndex]);
    const proximasRutinas = useMemo(() => rutinas.filter(r => r.dia !== todayIndex), [rutinas, todayIndex]);
    const rutinasVisibles = useMemo(() => mostrarTodas ? proximasRutinas : proximasRutinas.slice(0, 2), [mostrarTodas, proximasRutinas]);
    ...
    const Card = ({ children, className, onClick }) => (
        <div className={`bg-gray-800 rounded-xl p-4 shadow-md ${className}`} onClick={onClick} role={onClick ? "button" : undefined} tabIndex={onClick ? 0 : undefined}>{children}</div>
    );
                                    ...
                                    <div className="flex flex-col justify-between h-full">
                                        ...
                                        {rutinaHoy.isCompleted ? (
                                            <div className="mt-4 flex items-center gap-2 text-green-400 font-bold py-2 px-3 rounded-lg bg-gray-700 text-sm">
                                                <FaCheckCircle className="text-lg" /><span>Completado</span>
                                            </div>
                                        ) : (
                                            <button onClick={() => iniciarRutina(rutinaHoy)} className="mt-4 w-full flex items-center justify-center gap-2 bg-cyan-400 text-gray-900 font-bold py-2 px-4 rounded-lg shadow-lg hover:bg-cyan-300 transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-cyan-400/50">
                                                <FaPlayCircle className="text-lg" /><span>Iniciar</span>
                                            </button>
                                        )}
                                    </div>
                                </Card>
                                    ...
                                    <AnimatePresence>
                                        {rutinasVisibles.map(rutina => (
                                            <motion.div key={rutina.dia} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                                <Card className="flex items-center gap-3 hover:bg-gray-700 bg-gray-800/20 backdrop-blur-md rounded-xl transition-colors duration-200" onClick={() => iniciarRutina(rutina)}>
                                                    <div>
