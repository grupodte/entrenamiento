...
const DashboardRutinas = () => {
    ...
    const iniciarRutina = (rutina) => {
        // Modificado para navegar a la nueva ruta de selección de orden de bloques
        navigate(`/rutina/${rutina.rutinaId}/orden`, {
            state: { tipo: rutina.tipo, startTimer: true }
        });
    }

    // Esta función ya no es necesaria si siempre pasamos por la selección de orden
    // Si se quiere mantener la opción de ver detalle sin iniciar, se puede ajustar
    // const verDetalleRutina = (rutina) => {
    //     navigate(`/rutina/${rutina.rutinaId}?tipo=${rutina.tipo}`);
    // }

    // Temporalmente comento la función verDetalleRutina, ya que la lógica de "Iniciar Entrenamiento"
    // ahora cubre ambos casos (rutinas de hoy y próximos días) y los dirige a la selección de orden.
    // Si se necesita una funcionalidad separada para "ver detalle" sin iniciar, podemos reintroducirla.
    // Por ahora, el botón de los próximos días también usará iniciarRutina.

    // Ya no se usa verDetalleRutina, el botón de "Próximos Días" también usará iniciarRutina
    // para mantener la consistencia del flujo de iniciar un entrenamiento.
    // const verDetalleRutina = (rutina) => {
    //     navigate(`/rutina/${rutina.rutinaId}?tipo=${rutina.tipo}`);
    // }
